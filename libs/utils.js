...
const handleParcel = async () => {  
  ...
  if (answers.installParcel) {
    log.rawrs('Installing parcel, please wait');
    execa.shellSync('npm install -g parcel-bundler');
    log.success('Parcel installed successfully.');
  } else {
...
const hasValidArgs = (type, args) => {
  if (type === 'component') {
    if (args.functional && args.redux) {
      log.warn('You cannot use a redux in a functional component, so the redux requirement has been abandoned.\n\nYou can destroy this component with  "rawrs destroy" and run your command again without the -f flag to make a stateful, redux connected class component.');
      return true;
    }
